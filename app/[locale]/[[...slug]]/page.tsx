...
const paths = [[], ["pricing"], ["queue"], ["resources"], ["blog", "powerpoint-slide-size"], ["templates"], ["templates", "business"], ...templates.map(({ slug }) => ["templates", slug])] as string[][];
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug?: string[] }> }): Promise<Metadata> {
  ...
  if (path === "templates" || path === "templates/business" || path.startsWith("templates/")) {
    ...
    const title = template ? `${localizedTemplateName(template, locale).name} | FreeAIPPT` : path === "templates/business" ? `${copy.businessTitle1} ${copy.businessTitle2} | FreeAIPPT` : `${copy.badge} | FreeAIPPT`;
    const route = `/${path}`;
    return { title, description: template?.description ?? (path === "templates/business" ? copy.businessLead : copy.lead), alternates: { canonical: localizePath(locale, route), languages: { ...languageAlternates(route), "x-default": route } } };
  }
...
export default async function LocalePage({ params }: { params: Promise<{ locale: string; slug?: string[] }> }) {
  ...
  const path = (slug ?? []).join("/");
  if (path === "templates" || path === "templates/business" || (path.startsWith("templates/") && getTemplate((slug ?? [])[1]))) return <LocalizedTemplates locale={locale} slug={slug ?? []} />;
  const page = pageFromSlug(slug);
  if (!page) notFound();
