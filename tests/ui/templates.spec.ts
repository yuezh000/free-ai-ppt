...
test("template directory exposes twelve focused presentation categories", async ({ page }) => {
  ...
  await expect(page.getByTestId("template-category-business-plan")).toContainText("4 free templates");
  await expect(page.getByTestId("template-category-artificial-intelligence")).toBeVisible();
  await expect(page.locator(".template-card")).toHaveCount(48);
  ...
  await page.getByRole("heading", { level: 3, name: "Free Digital Marketing Launch Plan PowerPoint Template", exact: true }).getByRole("link").click();
  await expect(page).toHaveURL(/\/templates\/digital-channel-launch-plan\/?$/);
  await expect(page.getByTestId("template-digital-channel-launch-plan-download")).toHaveAttribute("href", "/templates/files/digital-channel-launch-plan.pptx");
});
...
test("localized template category links preserve the selected locale", async ({ page }) => {
  ...
  await page.getByRole("heading", { level: 3, name: "免费依赖日历产品路线图 PowerPoint 模板", exact: true }).getByRole("link").click();
  await expect(page).toHaveURL(/\/zh-CN\/templates\/dependency-calendar-product-roadmap\/?$/);
  ...
  await expect(page.locator('link[rel="alternate"][hreflang="zh-CN"]')).toHaveAttribute("href", "https://freeaippt.space/zh-CN/templates/dependency-calendar-product-roadmap");
  await expect(page.getByText("由 FreeAIPPT 设计并审核", { exact: true })).toBeVisible();
  await expect(page.getByTestId("template-dependency-calendar-product-roadmap-provenance")).toContainText("原创设计，制作过程透明");
  await expect(page.getByRole("heading", { name: "常见问题", exact: true })).toBeVisible();
  await expect(page.getByTestId("template-dependency-calendar-product-roadmap-download")).toHaveAttribute("href", "/templates/files/dependency-calendar-product-roadmap.pptx");
});
